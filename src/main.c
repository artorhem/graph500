...
int main(int argc, char** argv) {
  ...
  tuple_graph tg;
  tg.nglobaledges = (int64_t)(edgefactor) << SCALE;  // 1048576
  ...
  if (!tg.data_in_file || tg.write_file) {
    ...
    if (cart_comm != MPI_COMM_NULL) {
      ...
      for (block_idx = 0; block_idx < block_limit; ++block_idx) { // block limit is 1
        ...
        packed_edge* actual_buf =
            (!tg.data_in_file && block_idx % ranks_per_row == my_col)
                ...
                : buf; //packed_edge *actual_buf = buf
#ifdef SSSP
        float* actual_wbuf =
            (!tg.data_in_file && block_idx % ranks_per_row == my_col)
        ...
        fprintf(stderr,
                ...
                (int64_t)(start_edge_index + edge_count),
                (my_col == (block_idx % ranks_per_row)) ? "" : "not ",
                (int64_t)(FILE_CHUNKSIZE * (block_idx / ranks_per_row)));
        ...
        if (tg.write_file) {
          ...
          printf("----------------------------------------%d %s %d ",
                 start_edge_index, actual_buf, edge_count);
